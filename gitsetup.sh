...
response=$(curl --write-out %{http_code} --silent --output /dev/null https://github.com/game-of-mates/schema)
...
if [[ "$response" -ne 200 ]] ; then
  printf "game-of-mates/schema repository has not been created .. trying to create using git creds"
  
  printf "game-of-mates/schema repository has not been created - to create try using either of these"
  printf "User repo: curl -u 'npiper:GIT_PASSWORD' https://api.github.com/user/repos -d '{\"name\":${artifactId}\"}'"
  printf "Org repo: curl -u 'npiper:GIT_PASSWORD' https://api.github.com/orgs/game-of-mates/repos -d '{\"name\":${artifactId}\"}'"
  exit 1;
fi
...
git remote add origin https://github.com/game-of-mates/schema.git
